...
func newAccessor(receiverType, fieldName, fieldType, convertType, zeroValue string) *accessor {
	return &accessor{
...
func (t *templateData) addIdent(x *ast.Ident, receiverType, fieldName string) {
	...
	t.Accessors = append(t.Accessors, newAccessor(receiverType, fieldName, x.String(), convertType, zeroValue))
}
...
const source = `/*
 THIS FILE IS AUTOMATICALLY GENERATED BY gen-accessors; DO NOT EDIT.
 * Datadog API for Go
 *
 * Please see the included LICENSE file for licensing information.
 *
...
func ({{.ReceiverVar}} *{{.ReceiverType}}) Get{{.FieldName}}() {{if .ConvertType}}{{.ConvertType}}{{else}}{{.FieldType}}{{end}} {
  if {{.ReceiverVar}} == nil || {{.ReceiverVar}}.{{.FieldName}} == nil {
    return {{.ZeroValue}}
  }
  return {{if .ConvertType}}{{.ConvertType}}({{end}}*{{.ReceiverVar}}.{{.FieldName}}{{if .ConvertType}}){{end}}
}

// GetOk{{.FieldName}} returns a tuple with the {{.FieldName}} field if it's non-nil, zero value otherwise
// and a boolean to check if the value has been set.
func ({{.ReceiverVar}} *{{.ReceiverType}}) Get{{.FieldName}}Ok() ({{if .ConvertType}}{{.ConvertType}}{{else}}{{.FieldType}}{{end}}, bool){
  if {{.ReceiverVar}} == nil || {{.ReceiverVar}}.{{.FieldName}} == nil {
    return {{.ZeroValue}}, false
  }
  return {{if .ConvertType}}{{.ConvertType}}({{end}}*{{.ReceiverVar}}.{{.FieldName}}{{if .ConvertType}}){{end}}, true
}
...
func ({{.ReceiverVar}} *{{.ReceiverType}}) Set{{.FieldName}}(v {{if .ConvertType}}{{.ConvertType}}{{else}}{{.FieldType}}{{end}}) {
  {{if .ConvertType}}val := {{.FieldType}}(v)
  {{end}}{{.ReceiverVar}}.{{.FieldName}} = &{{if .ConvertType}}val{{else}}v{{end}}
}
